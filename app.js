...
app.get('/images', async (req, res) => {
  try {
    ...
    html += `
      <h2 class="text-2xl font-semibold mb-4">General Uploads</h2>
  ...
  pythonProcess.on('close', (code) => {
    ...
    if (code === 0 && !errorOutput) {
      res.send(`
        ...
        <div class="flex-grow flex items-center justify-center">
          <div class="bg-gray-800 p-8 rounded-xl shadow-2xl max-w-md w-full text-center">
            <h1 class="text-3xl font-bold mb-4 text-green-400">Script Executed Successfully</h1>
            <pre class="text-left mb-6 bg-gray-700 p-4 rounded-lg overflow-auto">${output || 'No output from script'}</pre>
            <a href="/" class="inline-block bg-blue-600 hover:bg-blue-500 text-white font-semibold py-2 px-6 rounded-lg transition duration-200">Back to Main Page</a>
          </div>
        </div>
        </body>
      ...
      res.status(500).send(`
        ...
        <div class="flex-grow flex items-center justify-center">
          <div class="bg-gray-800 p-8 rounded-xl shadow-2xl max-w-md w-full text-center">
            <h1 class="text-3xl font-bold mb-4 text-red-400">Script Execution Failed</h1>
            <p class="mb-4">Exit code: ${code}</p>
            <pre class="text-left mb-6 bg-gray-700 p-4 rounded-lg overflow-auto">${errorOutput || output || 'Unknown error occurred'}</pre>
            <a href="/" class="inline-block bg-blue-600 hover:bg-blue-500 text-white font-semibold py-2 px-6 rounded-lg transition duration-200">Back to Main Page</a>
          </div>
        </div>
        </body>
